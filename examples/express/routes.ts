...
import { JSForceConnOptions, performBulkQuery } from './sfdc';

const applicationRoutes: Router = Router();

applicationRoutes.get('/download', generateFile2);
applicationRoutes.get('/sfdc', downloadSFDCRecords);
...
function downloadSFDCRecords(req: Request, res: Response) {
  logger.info('generateFile');
  ...
  const connOpts: JSForceConnOptions = {
    loginUrl: process.env.SFDC_LOGIN_URL as string,
    username: process.env.SFDC_USERNAME as string,
    password: process.env.SFDC_PASSWORD as string,
  };
  const soqlQuery: string = process.env.SFDC_SOQL as string;

  performBulkQuery(connOpts, soqlQuery, xlsxWriter)
    .then(() => xlsxWriter.commit())
    .catch(err => logger.error(err));
